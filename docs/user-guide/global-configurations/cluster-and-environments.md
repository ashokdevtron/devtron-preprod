...
* [Isolated Cluster](cluster-and-environments.md#add-isolated-cluster) - For air-gapped use-cases, use this option.
...
Environment deletion is not allowed if any application has a CD pipeline corresponding to the environment. In such a case, go to [Workflow Editor](../creating-application/workflow/) and delete the deployment pipeline first, and then return to delete the environment. This action is irreversible, so make sure no critical applications or resources depend on the environment before deleting.
{% endhint %}
